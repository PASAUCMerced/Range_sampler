...
void RandomEngine::YSYChoice(
  ...
  if (replace) {
    ...
    if (num <
      ...
      //   IdxType j = RangeInt(i, population);
      //   std::swap(seq[i], seq[j]);
      // }
      // Save the rangely sampled numbers.
      for (IdxType i = 0; i < num; i++) {
        out[i] = seq[i];
      }
    }
...
template void RandomEngine::YSYChoice<int32_t>(
    int32_t num, int32_t population, int32_t* out, bool replace);
template void RandomEngine::YSYChoice<int64_t>(
