...
template <DGLDeviceType XPU, typename IdType>
COOMatrix _CSRRowWiseSamplingUniformYSY(
    CSRMatrix mat, IdArray rows, const int64_t num_picks, const bool replace) {
  const auto& ctx = rows->ctx;
  auto device = runtime::DeviceAPI::Get(ctx);
  cudaStream_t stream = runtime::getCurrentCUDAStream();
...
COOMatrix CSRRowWiseSamplingUniformYSY(
  ...
  if (num_picks == -1) {
    ...
    return _CSRRowWiseSamplingUniformYSY<XPU, IdType>(
        mat, rows, num_picks, replace);
  }
}

template COOMatrix CSRRowWiseSamplingUniformYSY<kDGLCUDA, int32_t>(
    CSRMatrix, IdArray, int64_t, bool);
template COOMatrix CSRRowWiseSamplingUniformYSY<kDGLCUDA, int64_t>(
    CSRMatrix, IdArray, int64_t, bool);
